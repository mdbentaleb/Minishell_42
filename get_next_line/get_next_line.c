...
/*   Updated: 2025/06/10 11:10:20 by moben-ta         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*dikchi_lib9a(char *rest)
{
	...
	tmp = malloc(sizeof(char) * (ft_strlen_get(rest + i) + 1));
	if (!tmp)
		return (NULL);
	while (rest[i])
		tmp[j++] = rest[i++];
